...
def load_data():
    try:
        ...
        required_columns = [
            ...
            '유효카드수_신용', '유효카드수_체크', '이용금액_일시불_B0M',
            '이용금액_할부_B0M', '이용건수_일시불_B0M', '카드이용한도금액',
            '입회경과개월수_신용', '_1순위업종', '_1순위업종_이용금액',
            '_2순위업종', '_2순위업종_이용금액', '이용금액_온라인_R6M',
            '이용금액_오프라인_R6M', '이용건수_온라인_R6M', '이용건수_오프라인_R6M'
        ]
        ...
        with sub_tabs[0]:  # 세그먼트 개요
            ...
            if segment_count > 0:
                ...
                if '입회경과개월수_신용' in df.columns:
                    ...
                    # 백업: 시뮬레이션 데이터 (실제 데이터가 없는 경우)
                    segment_data['가입기간_년'] = np.random.normal(2, 1, len(segment_data))  # 2년 평균
                    segment_stats['avg_membership_years'] = 2.0  # 기본값
                    total_stats['avg_membership_years'] = 2.0
