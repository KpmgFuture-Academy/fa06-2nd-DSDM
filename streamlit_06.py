    ...
    if current_main_tab == "세그먼트 분석":
        ...
        with sub_tabs[0]:  # 주요 KPI 비교
            ...
            def calculate_kpi_metrics(df_filtered, stat_type, amount_unit):
                # 세그먼트별 집계 (실제 존재하는 컬럼명 사용) - 개선된 방식
                g = df_filtered.groupby("Segment", as_index=False).agg(
                    고객수=("ID", "nunique"),
                    총청구금액=("청구금액_B0", "sum"),  # ARPU 계산용 (천원 단위)
                    일시불=("이용금액_일시불_B0M", "sum"),
                    할부=("이용금액_할부_B0M", "sum"),
                    현금서비스=("잔액_현금서비스_B0M", "sum"),
                    카드론=("잔액_카드론_B0M", "sum"),
                    총이용건수_일시불=("이용건수_일시불_B0M", "sum"),
                    총이용건수_할부=("이용건수_할부_B0M", "sum"),
                    총이용건수_신용=("이용건수_신용_B0M", "sum"),
                    총이용건수_체크=("이용건수_체크_B0M", "sum"),
                )
                ...
                g["총이용건수"] = g["총이용건수_일시불"] + g["총이용건수_할부"] + g["총이용건수_신용"] + g["총이용건수_체크"]
                
                # ARPU 계산 (천원 단위를 원 단위로 변환 후 계산)
                # 청구금액_B0는 천원 단위이므로 ×1000으로 원 단위 변환
                g["ARPU"] = (g["총청구금액"] * 1000) / g["고객수"]  # 원 단위 ARPU
                ...
                # 금액 단위는 천원으로 통일 (데이터셋이 천원 단위이므로 변환 없음)
                # g["총이용금액"] = g["총이용금액"]  # 천원 단위
                # g["일시불"] = g["일시불"]  # 천원 단위
                # g["할부"] = g["할부"]  # 천원 단위
                # g["현금서비스"] = g["현금서비스"]  # 천원 단위
                # g["카드론"] = g["카드론"]  # 천원 단위
                # g["ARPU"] = g["ARPU"]  # 천원 단위
                # g["객단가"] = g["객단가"]  # 천원 단위
        ...
        with sub_tabs[0]:  # 승인거절 분석
            ...
            with filter_col1:
                ...
                selected_segments = st.multiselect("세그먼트 선택", segments, 
                                                 default=segments, key="rejection_analysis_segments")
            
            with filter_col2:
                st.markdown("**지역**")
                regions = df['거주시도명'].unique().tolist()
                selected_regions = st.multiselect("지역 선택", regions, 
                                                default=regions, key="rejection_analysis_regions")
            ...
            if not filtered_df.empty:
                ...
                def calculate_rejection_metrics(df_filtered):
                    # 세그먼트별 승인거절 집계
                    rejection_data = df_filtered.groupby("Segment", as_index=False).agg(
                        고객수=("ID", "nunique"),
                        ...
                        총청구금액=("청구금액_B0", "sum")  # ARPU 계산용
                    )
                    
                    # 거절률 계산 (정확한 분모 사용)
                    rejection_data["거절률"] = rejection_data["총거절건수"] / rejection_data["승인요청건수"]
                    rejection_data["거절률"] = rejection_data["거절률"].fillna(0)
                    
                    # 거절률 sanity check (>1이면 에러)
                    rejection_data["거절률"] = rejection_data["거절률"].apply(lambda x: min(x, 1.0) if x > 1.0 else x)
                    
                    # 거절 사유별 비율 계산
                    rejection_data["한도초과비율"] = rejection_data["한도초과거절"] / rejection_data["총거절건수"]
                    rejection_data["입력오류비율"] = rejection_data["입력오류거절"] / rejection_data["총거절건수"]
                    rejection_data["한도초과비율"] = rejection_data["한도초과비율"].fillna(0)
                    rejection_data["입력오류비율"] = rejection_data["입력오류비율"].fillna(0)
                    
                    # ARPU 계산 (천원 -> 원 변환)
                    rejection_data["ARPU"] = (rejection_data["총청구금액"] * 1000) / rejection_data["고객수"]
                    rejection_data["ARPU"] = rejection_data["ARPU"].fillna(0)
                    
                    # 컬럼명 정리
                    rejection_data = rejection_data.rename(columns={"Segment": "세그먼트"})
        ...
        with sub_tabs[1]:  # 연체 현황
            ...
            with filter_col1:
                st.markdown("**세그먼트**")
                selected_segments = st.multiselect("세그먼트 선택", ['A', 'B', 'C', 'D', 'E'], 
                                                 default=['A', 'B', 'C', 'D', 'E'], key="delinquency_analysis_segments")
            
            with filter_col2:
                st.markdown("**지역**")
                regions = df['거주시도명'].unique().tolist()
                selected_regions = st.multiselect("지역 선택", regions, 
                                                default=regions, key="delinquency_analysis_regions")
        ...
        with sub_tabs[2]:  # 리볼빙/현금서비스
            ...
            with filter_col1:
                st.markdown("**세그먼트**")
                selected_segments = st.multiselect("세그먼트 선택", ['A', 'B', 'C', 'D', 'E'], 
                                                 default=['A', 'B', 'C', 'D', 'E'], key="revolving_analysis_segments")
            
            with filter_col2:
                st.markdown("**지역**")
                regions = df['거주시도명'].unique().tolist()
                selected_regions = st.multiselect("지역 선택", regions, 
                                                default=regions, key="revolving_analysis_regions")
            ...
            if not filtered_df.empty:
                ...
                def calculate_revolving_metrics(df_filtered):
                    # 세그먼트별 리볼빙/현금서비스 집계
                    revolving_data = df_filtered.groupby("Segment", as_index=False).agg(
                        고객수=("ID", "nunique"),
                        리볼빙이용고객수=("RV_평균잔액_R3M", lambda x: (x > 0).sum()),  # 리볼빙 이용율 계산
                        ...
                        총체크이용금액=("이용건수_체크_B0M", "sum"),  # 체크카드는 건수로 대체
                        총카드론이용금액=("잔액_카드론_B0M", "sum")  # 카드론은 잔액으로 대체
                    )
                    
                    # 신판 이용금액 합계 계산
                    revolving_data["총신판이용금액"] = revolving_data["총신판이용금액_일시불"] + revolving_data["총신판이용금액_할부"]
                    
                    # 리볼빙 이용율 계산
                    revolving_data["리볼빙이용율"] = revolving_data["리볼빙이용고객수"] / revolving_data["고객수"]
                    revolving_data["리볼빙이용율"] = revolving_data["리볼빙이용율"].fillna(0)
                    
                    # 컬럼명 정리
                    revolving_data = revolving_data.rename(columns={"Segment": "세그먼트"})
                    
                    return revolving_data
        ...
        with sub_tabs[3]:  # 한도/FDS
            ...
            with filter_col1:
                ...
                segments = ['A', 'B', 'C', 'D', 'E']
                selected_segments = st.multiselect("세그먼트 선택", segments, 
                                                 default=segments, key="limit_fds_analysis_segments")
            
            with filter_col2:
                st.markdown("**지역**")
                regions = df['거주시도명'].unique().tolist()
                selected_regions = st.multiselect("지역 선택", regions, 
                                                default=regions, key="limit_fds_analysis_regions")
            ...
            if not filtered_df.empty:
                ...
                def calculate_limit_fds_metrics(df_filtered):
                    # 세그먼트별 한도/FDS 집계
                    limit_data = df_filtered.groupby("Segment", as_index=False).agg(
                        고객수=("ID", "nunique"),
                    ...
                    # 한도소진율 계산 (신판 기준)
                    limit_data["한도소진율"] = limit_data["평균신판한도소진율"]
                    limit_data["한도소진율"] = limit_data["한도소진율"].fillna(0)
                    
                    # 한도초과 거절 비중 계산
                    limit_data["한도초과거절비중"] = limit_data["총한도초과거절"] / limit_data["총승인거절"]
                    limit_data["한도초과거절비중"] = limit_data["한도초과거절비중"].fillna(0)
                    
                    # FDS 근사 지표 계산 (한도초과 거절 비중으로 대체)
                    limit_data["FDS거절비중"] = limit_data["총한도초과거절"] / limit_data["총승인거절"]
                    limit_data["FDS거절비중"] = limit_data["FDS거절비중"].fillna(0)
                    
                    # 컬럼명 정리
                    limit_data = limit_data.rename(columns={"Segment": "세그먼트"})
                ...
                with col4:
                    st.metric(
                        label="총 한도심사요청",
                        value=f"{limit_df['총한도심사요청'].sum():,}건"
                    )
