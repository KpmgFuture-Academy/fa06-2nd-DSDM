# app_churn.py
# -----------------------------------------------------------------------------
# 이탈 예측 결과 시각화 대시보드 (CSV 직접 로드 버전)
# - 실행기(YAML+모델) 불필요
# - 이미 생성된 결과 CSV (risk_label_pred, risk_label_text, proba_* 포함)를 로드
...
st.set_page_config(page_title="이탈 예측 결과 대시보드", page_icon="📊", layout="wide")

# =========================
# 사이드바 입력
# =========================
st.sidebar.title("📊 이탈 예측 결과 뷰어")
csv_path = st.sidebar.text_input("결과 CSV 경로", value="test_predictions_with_labels.csv")
...
with tabs[0]:
    ...
    if st.session_state.pred_df is not None:
        ...
        if "risk_label_text" not in df.columns:
            st.error("CSV에 'risk_label_text' 컬럼이 없습니다.")
        else:
            summary = df["risk_label_text"].value_counts().reindex(
            ...
            # ✅ 자동으로 첫 번째 컬럼명을 잡아 x로 사용
            df_plot = summary.reset_index(name="count")
            x_col = df_plot.columns[0]  # 'index' 또는 'risk_label_text'
...
with tabs[1]:
    ...
    if st.session_state.pred_df is not None:
        df = st.session_state.pred_df
        label_col = "risk_label_text"
        num_feats = [
            c
            for c in df.columns
            if pd.api.types.is_numeric_dtype(df[c]) and c not in ["risk_label_pred"]
        ]
...
with tabs[2]:
    ...
    if st.session_state.pred_df is not None:
        st.dataframe(st.session_state.pred_df.head(1000), width="stretch", height=420)
        st.write("**기술 통계 (수치형)**")
        st.dataframe(st.session_state.pred_df.describe().T, width="stretch")
